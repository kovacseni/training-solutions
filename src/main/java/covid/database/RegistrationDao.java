...
public class RegistrationDao {
    ...
    private PriorityQueue<Person> getList(PreparedStatement stmt) throws SQLException {
        PriorityQueue<Person> vaccinationList = new PriorityQueue();
        try (ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
    ...
    private Person executeGetPerson(PreparedStatement stmt) throws SQLException {
        try (ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
                return getPerson(rs);
            }
    ...
    private Person getPerson(ResultSet rs) throws SQLException {
        ...
        VaccinationState numberOfVaccinesGot = VaccinationState.getState(rs.getInt(7));

        Person person = new Person(id, name, postalCode, age, email, ssn, numberOfVaccinesGot);
        if (rs.getDate(8) != null) {
            person.setLastVaccination(rs.getDate(8).toLocalDate());
            person.setVaccineType(VaccineType.valueOf(rs.getString(9)));
        }
    ...
    public void registrateVaccinationIntoDatabase(String ssn, VaccinationState numberOfVaccinesGot, LocalDate date, VaccineType vaccineType) throws SQLException {
        ...
        PreparedStatement stmt = conn.prepareStatement("UPDATE citizens SET number_of_vaccination = ?, last_vaccination = ?, vaccine_type = ? WHERE taj = ?;")) {
            ...
            stmt.setString(3, vaccineType.toString());
            stmt.setString(4, ssn);

            stmt.executeUpdate();
        }

        registrateVaccination(ssn, numberOfVaccinesGot, date, vaccineType);
    }
    ...
    public List<String> getPostalCodes() throws SQLException {
        ...
        ResultSet rs = stmt.executeQuery("select * from postal_codes;")) {
            while (rs.next()) {
                String town = rs.getString(1) + "," + rs.getString(2) + "," + rs.getString(3);
                postalCodes.add(town);
            }
        }
        return postalCodes;
    }
